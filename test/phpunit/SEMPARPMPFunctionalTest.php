...
/**
 * \file    test/phpunit/SEMPARPMPFunctionalTest.php
 * \ingroup semparpmp
 ...
 *
 * @package Testsemparpmp
 */
class SEMPARPMPFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modSEMPARPMP')->click();
		} else {
			// Disable the module
			$this->byHref('modSEMPARPMP')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/semparpmp/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('semparpmp/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/semparpmp/admin/about.php');
		$this->authenticate();
		return $this->assertContains('semparpmp/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('semparpmpwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modSEMPARPMP_SEMPARPMPTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modSEMPARPMP_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
